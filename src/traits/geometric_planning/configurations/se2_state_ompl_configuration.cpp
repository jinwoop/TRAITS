...
namespace nlohmann
{
    std::shared_ptr<const traits::Se2StateOmplConfiguration>
    adl_serializer<std::shared_ptr<const traits::Se2StateOmplConfiguration>>::from_json(const json& j)
    {
        return adl_serializer<std::shared_ptr<traits::Se2StateOmplConfiguration>>::from_json(j);
    }

    std::shared_ptr<traits::Se2StateOmplConfiguration>
    adl_serializer<std::shared_ptr<traits::Se2StateOmplConfiguration>>::from_json(const json& j)
    {
        ...
        const float yaw = j.at(traits::constants::k_yaw);

        return std::make_shared<traits::Se2StateOmplConfiguration>(x, y, yaw);
    }

    void adl_serializer<std::shared_ptr<const traits::Se2StateOmplConfiguration>>::to_json(
            json& j,
            const std::shared_ptr<const traits::Se2StateOmplConfiguration>& c)
    {
        ...
        j[traits::constants::k_yaw]                = c->yaw();
    }

    void adl_serializer<std::shared_ptr<traits::Se2StateOmplConfiguration>>::to_json(
            json& j,
            const std::shared_ptr<traits::Se2StateOmplConfiguration>& c)
    {
        adl_serializer<std::shared_ptr<const traits::Se2StateOmplConfiguration>>::to_json(j, c);
    }
}  // namespace nlohmann
