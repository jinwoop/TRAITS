...
{
    template <>
    struct adl_serializer<std::shared_ptr<const traits::ConfigurationBase>>
    {
        //! Non-default constructable from_json
        static std::shared_ptr<const traits::ConfigurationBase> from_json(const json& j);
        //! to_json
        static void to_json(json& j, const std::shared_ptr<const traits::ConfigurationBase>& c);
    };

    template <>
    struct adl_serializer<std::shared_ptr<traits::ConfigurationBase>>
    {
        //! Non-default constructable from_json
        static std::shared_ptr<traits::ConfigurationBase> from_json(const json& j);
        //! to_json
        static void to_json(json& j, const std::shared_ptr<traits::ConfigurationBase>& c);
    };
}  // namespace nlohmann
