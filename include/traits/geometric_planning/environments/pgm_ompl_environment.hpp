...
#include <memory>
// Local
#include "traits/common/utilities/pgm.hpp"
#include "traits/geometric_planning/environments/ompl_environment.hpp"

namespace traits
{
    //! Environment where the map comes from a PGM image
    class TraitsPgmOmplEnvironment : public TraitsOmplEnvironment
    {
    public:
        //! For json
        TraitsPgmOmplEnvironment();

        //! Constructor
        TraitsPgmOmplEnvironment(const std::string& filepath,
    ...
    private:
        ...
        friend void from_json(const nlohmann::json& j, TraitsPgmOmplEnvironment& e);
    };

    void from_json(const nlohmann::json& j, TraitsPgmOmplEnvironment& e);

    // Inline functions
    float TraitsPgmOmplEnvironment::minX() const
    {
        return m_origin_x;
    }

    float TraitsPgmOmplEnvironment::maxX() const
    {
        return m_origin_x + m_pgm.width() * m_resolution;
    }

    float TraitsPgmOmplEnvironment::minY() const
    {
        return m_origin_y;
    }

    float TraitsPgmOmplEnvironment::maxY() const
    {
        return m_origin_y + m_pgm.height() * m_resolution;
    }

    float TraitsPgmOmplEnvironment::resolution() const
    {
        return m_resolution;
    }

    std::pair<int, int> TraitsPgmOmplEnvironment::toCell(const float x, const float y) const
    {
