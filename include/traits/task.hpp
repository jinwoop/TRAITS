...
namespace traits {
    ...
    public:
        ...
        inline void setTotalDuration(float dynamic_task_duration) const; // mutable
        [[nodiscard]] inline float totalDuration() const;
        [[nodiscard]] inline float maximum_dynamic_task_duration() const;

        [[nodiscard]] inline float fastestSpeciesSpeed() const;
        [[nodiscard]] inline float slowestSpeciesSpeed() const;

        //! \returns The desired traits for the task
        [[nodiscard]] inline virtual const Eigen::VectorXf &desiredTraits() const;

        //! \returns The minimum trait rates for the task
        [[nodiscard]] inline virtual const Eigen::VectorXf &minimum_trait_rates() const;

        [[nodiscard]] inline virtual const Eigen::VectorXf &traits_aggregatable() const;
        ...
        [[nodiscard]] float computeDurationLBHeuristic() const;

    private:
        float m_fastest_species_speed;
        float m_slowest_species_speed;
        float m_maximum_dynamic_task_duration;
        mutable float m_total_task_duration;

        std::shared_ptr<SasAction> m_symbolic_action;
        Eigen::VectorXf m_desired_traits;
        Eigen::VectorXf m_minimum_trait_rates;
        Eigen::VectorXf m_traits_aggregatable;
    ...
    // Inline Functions
    float Task::fastestSpeciesSpeed() const
    {
       return m_fastest_species_speed;
    }
    float Task::slowestSpeciesSpeed() const
    {
    ...
    const Eigen::VectorXf& Task::minimum_trait_rates() const
    {
        return m_minimum_trait_rates;
    }

    const Eigen::VectorXf& Task::traits_aggregatable() const
    {
        return m_traits_aggregatable;
    }
    ...
    float Task::totalDuration() const
    {
        return m_total_task_duration;
    }

    void Task::setTotalDuration(float dynamic_task_duration) const
    {
        m_total_task_duration = dynamic_task_duration;
    }

    float Task::maximum_dynamic_task_duration() const
    {
        return m_maximum_dynamic_task_duration;
    }
}  // namespace traits