...
static void from_json(const json& j, Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>& m)
{
    ...
    {
        throw traits::createLogicError("malformed json for eigen vector or matrix");
    }
}
...
static void to_json(json& j, const Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>& m)
{
    ...
    {
        throw traits::createLogicError("Don't know how we got here...");
    }
}
};
...
{
    j[traits::constants::k_qw] = q.w();
    j[traits::constants::k_qx] = q.x();
    j[traits::constants::k_qy] = q.y();
    j[traits::constants::k_qz] = q.z();
}
...
static void to_json(json& j, const Eigen::QuaternionBase<Derived>& q)
{
    ...
    j.at(traits::constants::k_qw).get_to<Scalar>(q.w());
    j.at(traits::constants::k_qx).get_to<Scalar>(q.x());
    j.at(traits::constants::k_qy).get_to<Scalar>(q.y());
    j.at(traits::constants::k_qz).get_to<Scalar>(q.z());
}
};
