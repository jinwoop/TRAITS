...
{
    /*!
     * \brief Applies an edge from an undirected graph
     *
    ...
    {
        using __Edge = typename UndirectedGraphSearchNodeDeriv::Edge;

    public:
        //! Constructor
        explicit UndirectedGraphEdgeApplier(const std::shared_ptr<__Edge>& edge)
                : m_edge(edge)
        {}
