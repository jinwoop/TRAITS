...
namespace nlohmann
{
    //! Custom json serialization for traits::UndirectedGraphVertex
    template <typename T>
    struct adl_serializer<traits::UndirectedGraphVertex<T>>
{
    static void to_json(json& j, const traits::UndirectedGraphVertex<T>& vertex)
    {
        j = vertex.payload();
    }
};

//! Custom json serialization for traits::UndirectedGraphEdge
template <typename T>
struct adl_serializer<traits::UndirectedGraphEdge<T>>
{
    static void to_json(json& j, const traits::UndirectedGraphEdge<T>& edge)
    {
        j = {{traits::constants::k_cost, edge.cost()},
             {traits::constants::k_vertex_a, edge.vertexA()->id()},
             {traits::constants::k_vertex_b, edge.vertexB()->id()}};
    }
};
}  // namespace nlohmann
