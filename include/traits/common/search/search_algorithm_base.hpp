...
// Local
#include "traits/common/search/search_results.hpp"
...
namespace traits
{
    ...
    public:
        ...
        {
            TimerRunner timer_runner(m_parameters->get<std::string>(constants::k_timer_name));
            std::shared_ptr<SearchNode> root = createRootNode();
            return searchFromNode(root);
        }

    protected:
        //! \brief Default Constructor
        explicit SearchAlgorithmBase(const std::shared_ptr<const ParametersBase>& parameters)
                : m_statistics(std::make_shared<SearchStatistics>(parameters->get<std::string>(constants::k_timer_name)))
