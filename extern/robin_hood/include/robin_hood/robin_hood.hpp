        ...
        namespace detail_
        {
            ...
            struct IntSeqImpl
            {
                ...
                using TResult = typename IntSeqCombiner<
                    ...
                    typename IntSeqImpl<TValue, Begin + (End - Begin) / 2, End, (End - Begin + 1) / 2 == 1>::TResult>::
                    TResult;
            };
...
// make sure we static_cast to the correct type for hash_int
        ...
        class BulkPoolAllocator
        {
           public:
            ...
            {
                // compute number of available elements in ptr
           ...
           private:
            // iterates the list of allocated memory to compute how many to alloc next.
    ...
    struct pair
    {
        ...
        constexpr pair(U1&& a, U2&& b) noexcept(noexcept(T1(std::forward<U1>(std::declval<U1&&>()))) && noexcept(
...
#    pragma GCC diagnostic pop
    ...
    namespace detail
    {
        ...
        //
        // [NodeDeriv, NodeDeriv, ... NodeDeriv | info, info, ... infoSentinel ]
        //
        // * NodeDeriv: either a DataNode that directly has the std::pair<key, val> as member,
        //   or a DataNode with a pointer to std::pair<key,val>. Which DataNode representation to use
        //   depends on how fast the swap() operation is. Heuristically, this is automatically choosen
        //   based on sizeof(). there are always 2^n Nodes.
        //
        // * info: Each NodeDeriv in the map has a corresponding info byte, so there are 2^n info bytes.
        //   Each byte is initialized to 0, meaning the corresponding NodeDeriv is empty. Set to 1 means the
        //   corresponding node contains data. Set to 2 means the corresponding NodeDeriv is filled, but it
...
#    endif
            ...
            size_t findIdx(Other const& key) const
            {
                ...
                return mMask == 0 ? 0
                                  : static_cast<size_t>(
                                        std::distance(mKeyVals, reinterpret_cast_no_cast_align_warning<Node*>(mInfo)));
            }
           ...
           private:
            ...
            void initData(size_t max_elements)
            {
                ...
                ROBIN_HOOD_LOG("std::calloc " << numBytesTotal << " = calcNumBytesTotal(" << numElementsWithBuffer
                                              << ")")
                mKeyVals =
                    reinterpret_cast<Node*>(detail::assertNotNull<std::bad_alloc>(std::calloc(1, numBytesTotal)));
                mInfo = reinterpret_cast<uint8_t*>(mKeyVals + numElementsWithBuffer);
            ...
            bool increase_size()
            {
                ...
                ROBIN_HOOD_LOG("mNumElements="
                               << mNumElements << ", maxNumElementsAllowed=" << maxNumElementsAllowed << ", load="
